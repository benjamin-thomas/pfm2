			...
			.then((accountsResult) => {
				Result.match(
					...
					(accounts) => {
						...
						if (!unknownExpense) {
							throw new Error(
								"Data integrity error: Unknown_EXPENSE account not found",
							);
						}
						if (!unknownIncome) {
							throw new Error(
								"Data integrity error: Unknown_INCOME account not found",
							);
						}
				...
				switch (financialData.kind) {
					...
					case "Loaded": {
						...
						if (!unknownExpense || !unknownIncome) {
							throw new Error(
								"Data integrity error: Unknown accounts must exist",
							);
						}
						...
						const filteredLedgerEntries = ledgerEntries.filter((entry) => {
							...
							if (filters.minAmount) {
								const minCents = Math.round(
									parseFloat(filters.minAmount) * 100,
								);
							...
							if (filters.maxAmount) {
								const maxCents = Math.round(
									parseFloat(filters.maxAmount) * 100,
								);
							...
							return !(
								filters.unknownExpenses &&
								!isUnknownAccount(entry.toAccountName)
							);
						});
										...
										<div className="transaction-list__header">
											<div className="transaction-list__header-title">
												...
												<span className="transaction-count">
													{filteredLedgerEntries.length} of{" "}
